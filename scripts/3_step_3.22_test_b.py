...
def phase_0_tune_and_train_oracle_compressed(X_train, y_train, n_trials=30):
    ...
    print("[REDACTED_BY_SCRIPT]")
    probe = lgb.LGBMRegressor(random_state=42, n_jobs=-1, verbosity=-1, min_split_gain=0.01, n_estimators=500, learning_rate=0.001, num_leaves=11, max_depth=7, subsample=0.8, colsample_bytree=0.8, min_child_samples=20)
    ...
    def objective(trial):
        ...
        n_top = trial.suggest_int('n_top_features', 40, 400)
        ...
        tail_pipeline = Pipeline([
            ...
            ('pca', PCA(n_components=20, random_state=42, whiten=True)) # Whiten to normalize variance
        ])
        ...
        score = cross_val_score(
            ...
            X_fused, y_solar, 
            cv=TimeSeriesSplit(n_splits=TRIALS_GLOBAL), 
    ...
    tail_pipeline_final = Pipeline([
        ('imputer', SimpleImputer(strategy='median')),
        ('scaler', RobustScaler(quantile_range=(5.0, 95.0))), # Ignore extreme 5% outliers
        ('pca', PCA(n_components=n_pca_final, random_state=42, whiten=True)) # Whiten to normalize variance
    ])
    ...
    # Train Final Oracle
    final_oracle = lgb.LGBMRegressor(**best_params, random_state=42, n_jobs=-1, verbosity=-1)
...
def phase_J_train_stratified_capacity_residuals(X_train, y_residual_train, groups, n_trials=20):
    ...
    for stratum in labels:
        ...
        if n_groups_subset <= 1:
             ...
             cv_select = KFold(n_splits=5, shuffle=True, random_state=42)
    ...
    for stratum in labels:
        ...
        if w_subset is not None:
            X_opt_train, X_opt_val, y_opt_train, y_opt_val, w_opt_train, w_opt_val = train_test_split(
                X_subset, y_subset, w_subset, test_size=0.2, random_state=42
            )
        else:
            X_opt_train, X_opt_val, y_opt_train, y_opt_val = train_test_split(
                X_subset, y_subset, test_size=0.2, random_state=42
        ...
        def objective_rest(trial):
            # Same structure, slightly relaxed constraints for larger N
            params = {
...
def phase_I_train_solar_residual_specialist(X_train_comp, y_residual, n_trials=50):
    ...
    X_opt_train, X_opt_val, y_opt_train, y_opt_val = train_test_split(
        X_train_comp, y_residual, test_size=0.2, random_state=42
    )
...
if __name__ == '__main__':
    ...
    gm_train_val_df, gm_test_df = train_test_split(
        gm_solar_df, test_size=0.1, random_state=42,
    ...
    gm_train_df, gm_val_df = train_test_split(
        gm_train_val_df, test_size=0.05, # 0.15 / 0.85 = 0.1765
