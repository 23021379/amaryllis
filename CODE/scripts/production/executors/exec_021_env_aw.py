"""
exec_021_env_aw.py: Environmental Constraint Integration (Ancient Woodland)
"""
import logging
...
# This path assumes an L1 artifact has been created for Ancient Woodlands
...
def _load_and_prepare_data() -> gpd.GeoDataFrame:
    ...
    if 'aw_area_ha' not in gdf.columns:
        gdf['aw_area_ha'] = gdf.geometry.area / 10_000
    
    # MANDATE: The source data must have a column to distinguish types, e.g., 'THEME' or 'TYPE'
    # For this example, we assume a 'woodland_type' column with values 'ASNW' and 'PAWS'.
    # This needs to be created during the L1 artifact generation.
    if 'woodland_type' not in gdf.columns:
        logging.warning("'woodland_type'[REDACTED_BY_SCRIPT]")
        gdf['woodland_type'] = 'UNKNOWN'
...
def calculate_proximity_and_containment(sites_gdf: gpd.GeoDataFrame, constraint_gdf: gpd.GeoDataFrame, prefix: str) -> pd.DataFrame:
    ...
    prox_join = prox_join.rename(columns={'aw_area_ha': f'[REDACTED_BY_SCRIPT]'})
    ...
    features = prox_join[[f'[REDACTED_BY_SCRIPT]', f'[REDACTED_BY_SCRIPT]', 'woodland_type']].copy()
    ...
    # One-hot encode the type of the NEAREST woodland
    features[f'[REDACTED_BY_SCRIPT]'] = (features['woodland_type'] == 'ASNW').astype(int)
    features[f'[REDACTED_BY_SCRIPT]'] = (features['woodland_type'] == 'PAWS').astype(int)
    
    return features.drop(columns=['woodland_type'])
...
def calculate_multi_radius_density(sites_gdf: gpd.GeoDataFrame, constraint_gdf: gpd.GeoDataFrame, prefix: str) -> pd.DataFrame:
    ...
    for r_m in RISK_RADII_M:
        ...
        area_sum = intersected.groupby(intersected.index).agg(
            total_area=(f'intersected_area', 'sum')
        ).rename(columns={'total_area': f'[REDACTED_BY_SCRIPT]'})

        # Calculate counts (total and by type)
        counts = intersected.groupby(intersected.index).agg(
            total_count=('hex_id', 'size'),
            asnw_count=('woodland_type', lambda x: (x == 'ASNW').sum()),
            paws_count=('woodland_type', lambda x: (x == 'PAWS').sum())
        ).rename(columns={
            'total_count': f'[REDACTED_BY_SCRIPT]',
            'asnw_count': f'[REDACTED_BY_SCRIPT]',
            'paws_count': f'[REDACTED_BY_SCRIPT]'
        })
...
def calculate_density_ratios(df: pd.DataFrame, prefix: str) -> pd.DataFrame:
    """[REDACTED_BY_SCRIPT]"""
    # Ratio of 2km area to 20km area
...
def execute(master_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    aw_gdf = _load_and_prepare_data()

    # --- Feature Generation ---
    prox_features = calculate_proximity_and_containment(master_gdf, aw_gdf, 'aw')
    density_features = calculate_multi_radius_density(master_gdf, aw_gdf, 'aw')
    ...
    final_gdf = calculate_density_ratios(final_gdf, 'aw')
