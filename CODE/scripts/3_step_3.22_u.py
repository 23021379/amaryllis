...
def phase_F_tune_final_calibrator(oracle_preds_val, head_prob_preds_val, y_val_true, adjustment_range):
    """
    AD-AM-42: Implements a dual-factor, rules-based calibrator. This function
    uses the validation set to find optimal adjustment factors for under- and
    over-prediction probabilities independently via a 2D grid search.
    """
    print("[REDACTED_BY_SCRIPT]")
    
    # Average the risk probabilities from all specialist heads
    # head_prob_preds_val is a dict: {cohort_name: DataFrame[prob_0, prob_1, prob_2, prob_3, prob_4]}
    avg_probs = pd.concat(head_prob_preds_val.values()).groupby(level=0).mean()
    
    # Consolidate probabilities for 'under' and 'over' prediction regimes
    prob_under = avg_probs[0] + avg_probs[1] # P(Severe Under) + P(Moderate Under)
    prob_over = avg_probs[3] + avg_probs[4]  # P(Moderate Over) + P(Severe Over)

    best_rmse = float('inf')
    best_factor_under = 0
    best_factor_over = 0

    # Perform a 2D grid search for the two independent factors
    for factor_under in adjustment_range:
        for factor_over in adjustment_range:
            # The new, more flexible calibration rule
            adjustment = (prob_over * factor_over) - (prob_under * factor_under)
            ...
            if rmse < best_rmse:
                best_rmse = rmse
                best_factor_under = factor_under
                best_factor_over = factor_over
            
    print(f"[REDACTED_BY_SCRIPT]")
    print(f"[REDACTED_BY_SCRIPT]")
    print(f"[REDACTED_BY_SCRIPT]")
    return best_factor_under, best_factor_over
...
def phase_D5_all_heads_shap_analysis(X_test, cohorts, head_models_dir, output_dir, gcv, oracle_preds_test, top_n=20):
    ...
    for cohort_name in sorted(cohorts.keys()):
        ...
        # absolute SHAP values across all classes. This prevents the IndexError.
        if isinstance(shap_values, list):
        ...
        plt.figure()
        # SURGICAL INTERVENTION: Prevent IndexError when a cohort has fewer features
        # than the requested max_display count.
        dynamic_max_display = min(top_n, X_test_head_specialist.shape[1])
...
if __name__ == '__main__':
    ...
    # 5. Tune the Final Calibrator using the validation set predictions.
    adjustment_factor_under, adjustment_factor_over = phase_F_tune_final_calibrator(
        oracle_preds_val, head_prob_preds_val, y_val_gm, adjustment_range=np.arange(0, 251, 10)
    )

    # 6. Generate final predictions on the unseen test set.
    print("[REDACTED_BY_SCRIPT]")
    head_prob_preds_test = {}
    for cohort_name, features in cohorts.items():
        model_path = os.path.join(HEADS_V35_DIR, f"[REDACTED_BY_SCRIPT]")
        if os.path.exists(model_path):
            model = joblib.load(model_path)
            model_features = [f for f in features if f in X_test_gm.columns]
            if model_features:
                probs = model.predict_proba(X_test_gm[model_features])
                head_prob_preds_test[cohort_name] = pd.DataFrame(probs, index=X_test_gm.index)
    
    # 7. Apply the tuned calibration rule.
    avg_probs_test = pd.concat(head_prob_preds_test.values()).groupby(level=0).mean()
    prob_under_test = avg_probs_test[0] + avg_probs_test[1]
    prob_over_test = avg_probs_test[3] + avg_probs_test[4]
    final_adjustment = (prob_over_test * adjustment_factor_over) - (prob_under_test * adjustment_factor_under)
    final_prediction_calibrated = oracle_preds_test + final_adjustment

    # The final Arbiter model is now the pair of tuned adjustment factors. Persist them.
    calibration_artifact = {
        'factor_under': adjustment_factor_under,
        'factor_over': adjustment_factor_over
    }
    joblib.dump(calibration_artifact, MODEL_V35_PATH)
    print(f"[REDACTED_BY_SCRIPT]")
    ...
    dossier_content.append(
        "[REDACTED_BY_SCRIPT]"
        "[REDACTED_BY_SCRIPT]'Dual-Factor Calibration'[REDACTED_BY_SCRIPT]"
    )
